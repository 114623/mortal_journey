...
function formatGongfaSlots(slots: GongfaSlotsState, highlightName?: string): string {
  ...
  for (let i = 0; i < slots.length; i++) {
    ...
    // 阶层面直接暴露给 AI：功法阶层低于主角境界时修炼不产修为（见 state_preset）。
    const tierText = resolveGongfaTier(g.tier) ? `${resolveGongfaTier(g.tier)}阶·` : "";
    lines.push(`功法：${g.name}（${tierText}${g.grade}，第${mastery}层/10层）${marker}`);
  }
  return lines.length > 0 ? lines.join("\n") : "无";
}
