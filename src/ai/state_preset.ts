...
1. <thinking>思考分析</thinking>
   ...
   - 状态影响：本轮事件对主角造成了哪些影响？
     ...
     · 突破：是否触及突破任务或完成突破？
    - 主角记忆：本回合有没有值得主角记住的事（得失、恩怨、关键信息、重大选择、关系或境界变化）？有就写 1~2 条，没有就给空数组，不要硬凑。
...
9. NPC生成需要包含的信息：
  ...
  9.8 画像提示词 profile（对象，见「NPC画像提示词规则」）：新 NPC 必须给出 personality；在场且本轮有互动的 NPC 必须给出更新后的 memory（**无新增内容则省略 memory 字段**，不要把旧日志原样抄一遍）。
...
<NPC_NEARBY_TAG>[
  {
    ...
    "profile": {
      ...
      "memory": "0005年12月20日 17:00\n七玄门·外门药圃\n在外门药圃初遇韩立，嫌其木讷却认可能采到月光草的本事；曾因他替自己挡下执法堂责罚而欠下一份人情"
    },
    "hpPercent": 100,
    "mpPercent": 100
  }
...
1. 数量对齐（先点名，后填表）：
   ...
   1.7 【数量词必须落成条目·重要】正文写了确数的人数，就要有那么多条角色卡：
       "五名守山弟子"= 5 条、"三名散修"= 3 条、"两个护卫"= 2 条、"师兄弟二人"= 2 条。
       群体人物用**可区分**的 displayName：守山弟子甲 / 乙 / 丙 / 丁 / 戊，或 守山弟子（2）（3）……
   1.8 【禁止同名条目·硬约束】同一个 nearbyNpcs 数组里**不得出现两条相同的 displayName**——
       前端按名字识别人，同名的多条会被合并成一个人，等于白写。群体一律按 1.7 编号区分。
   1.9 反模式（出现即判定失败）：正文写了"五个人"却只输出 1 条角色卡（把一群人压成一条）；
       或把"众人""众人等"写成一个条目。宁可多建几张普通卡，也不要少建。
2. 修为对齐（境界不得自由发挥）：
...
2. 生成时机：
   2.1 新 NPC 首次登场：必须输出 personality，并按第 3 条格式写入首条 memory（首次相遇的情形与对主角的第一印象）。
   2.2 既有 NPC 本轮与主角有实质互动（对话/交手/交易/同行/目睹事件）：必须输出更新后的 memory——
       在旧日志**最上面追加**本轮的新条目，旧条目原样保留（见 3.4），不要整段重写成流水账。
   2.2b 【不要重复回吐旧日志·省输出】若本轮没有值得记的新内容（只是照面、没什么可记），
       **省略 memory 字段**即可，程序会沿用快照里的旧值——不要把整段旧日志原样再抄一遍。
       只有真的新增了条目时才输出 memory（此时旧条目也要一并带上，程序按整段替换）。
   2.3 既有 NPC 本轮仅被提及、未实际互动：可省略 profile。
   2.4 personality 只在性格发生实质转变时更新（如结仇后转为阴狠、被救后转为敬重）；日常互动不要改性格。
3. 记忆格式（硬约束）——按条写的三行日志，**新的在最上面**：
   3.1 一条 = 三行，条目之间空一行：
       第 1 行 · 时间：「0005年12月20日 17:00」（年月日 + 时，**分钟位恒写 00**，世界时间只到小时，禁止编造分钟）；
                 跨日 / 跨月的连续事件写成区间：「0001年05月17日-0001年05月18日」（区间不写时刻）。
                 时间取**本轮推进之后**的世界时间（当前世界时间 + 你输出的时间增量），不得写"某日""前些日子"。
       第 2 行 · 地点：该 NPC 当时所在的地点，自然语言点分，如「溪京城·振远镖局·院内」；确实未知写「—」。
       第 3 行 · 正文：以该 NPC 的视角记一件事，1~3 句，写清做了什么、为什么、惦记着什么。
   3.2 同一时刻只写一条；同一件事不要拆成多条重复记。一次互动通常 1 条，最多 2 条。
   3.3 只写该 NPC 亲身经历或确知的事，禁止写入主角独自经历的、该 NPC 无从知晓的内容；
       保留情绪与立场（如"记恨主角夺走灵药，但忌惮其背后的长老"），让后续剧情有抓手。
   3.4 **除非是超了字数限制要压缩，否则不要让 AI 更改已经记录的事**（最重要）：
       旧条目一律原样保留，只能新增，不能改——严禁修正旧条目的措辞、严禁把几条旧条目合并重写成一条、
       严禁调整旧条目的顺序、严禁替旧条目补内容。需要更正时，新增一条新条目说明，绝不回头改旧条目。
       唯一例外：超过 1000 字触发压缩时（见第 4 条），允许把旧条目**提炼压缩**（保留事实、缩短文字）；
       注意**压缩 ≠ 删除**：不得整条丢掉旧条目来凑字数，除非某条已被压到只剩无信息的流水账。
       （程序会逐条校验：未超阈值时，旧条目只要被改写或整条丢失，你这一版会被拒绝并回滚，等于白写。）
   3.5 **玩家手写的内容你动不得**：日志里那些**没有时间行的自由散文、玩家手动补写的句子**，
       一律原样保留——不得改写、不得删掉、也不得改写成三行格式。那是玩家自己写进去的东西，
       你没有权限动它；要补充就另起一条新条目写在最上面。
4. 长度与压缩（重要）：
   4.1 memory **没有硬上限**，可随经历一直累积——记住的恩怨、承诺、秘密越多，人物越立体。
   4.2 当快照中该 NPC 的现有 memory **超过 1000 字**时，本轮先把旧条目**提炼压缩**到 700 字左右，
       再在最上面追加本轮新条目。压缩 = 把冗长的旧条目压短（保留事实，删掉重复与废话），
       **不是把旧条目整条删掉**——发生过的事不能凭空消失。
   4.3 提炼的写法：每条仍保持三行结构（时间 / 地点 / 正文），正文压到一句；
       同一时段、内容相近的相邻条目可以合并成一条（合并后时间行写区间），这是唯一的"合并"合法场景。
       优先保留：与主角/重要人物的恩怨与承诺、未了的执念与秘密、性格转折的关键事件、当前惦记的事。
       优先压掉：重复的日常流水账、已被后续事件覆盖的旧情绪、与人物关系不大的见闻。
   4.4 只有超过 1000 字的那一轮才允许动旧条目；未超阈值时一个字都不改，只在最上面追加（见 3.4）。
5. 玩家锁定：若快照中该 NPC 名字后标注 [画像已锁定]，说明玩家已手动改写其画像，**禁止输出该 NPC 的 profile**。
6. 示例（memory 是三行一条的日志，新的在最上面，条目之间空一行）：
   "profile": {
...
2. 何时写：本回合确有值得记的事就写 1~2 条；平淡回合（纯赶路、无信息的寒暄）输出空数组「{"entries":[]}」，不要硬凑。
3. 只写主角亲身经历或确知的事，禁止写入主角无从知晓的内容（他人内心、幕后真相）。
4. **只追加不改**：除非超字数触发压缩，否则不得更改已有条目（同 NPC 记忆 3.4）。
5. 压缩：主角记忆超过 1000 字时，本轮必须压缩——输出「{"full":"提炼后的整段记忆（约700字，保留事实、压短文字，不是删条目）","entries":["本轮新条目"]}」；未超字数时只给 entries。
6. 输出格式：<MJ_PROTAGONIST_MEMORY_TAG> … </MJ_PROTAGONIST_MEMORY_TAG>，内为 JSON 对象（无内容时「{"entries":[]}」）。
7. **主角记忆只写这一段**：主角的性格（personality）与外貌（appearance）由玩家设定，你不得输出或改动。
