...
function placeEquipInto(npc: Npc, raw: unknown): void {
  ...
  const treasure = parseEquipObject(raw, npc.realm.major, npc.realm.minor, gradeRangeForPowerTier(npc.powerTier));
  if (emptyIdx >= 0) {
...
function placeGongfaInto(npc: Npc, raw: unknown, linggen?: string[]): void {
  ...
  const gongfa = parseGongfaObject(raw, npc.realm.major, npc.realm.minor, linggen, gradeRangeForPowerTier(npc.powerTier));
  if (emptyIdx >= 0) {
...
function placeInventoryInto(npc: Npc, raw: unknown, linggen?: string[]): void {
  if (!raw || typeof raw !== "object") return;
  const item = parseStorageObject(raw, npc.realm.major, npc.realm.minor, linggen, gradeRangeForPowerTier(npc.powerTier));
