...
import { parseStorageObject, parseGrade, rollGrade } from "../ai/parseAiItem";
...
import { createEmptyProfile, normalizeProfile } from "./types/playInfo";
import {
...
/**
 * 主角全局单例的 ref 容器（挂在 `globalThis` 上）。
 *
 * 挂在全局是为了扛住开发期热更新：vite HMR 重新执行本模块时若直接 `ref(null)`，
 * 会造出一个全新的空 ref，而未被重建的 UI 组件仍绑定旧 ref ——
 * 表现就是「主角凭空消失，左侧档案只剩占位文案」。复用同一个 ref 即可避免。
 */
const MJ_PROTAGONIST_REF_KEY = "__MJ_PROTAGONIST_REF__";
const globalHolder = globalThis as unknown as Record<string, unknown>;
const cachedProtagonistRef = globalHolder[MJ_PROTAGONIST_REF_KEY] as Ref<Protagonist | null> | undefined;
const protagonistRef: Ref<Protagonist | null> = cachedProtagonistRef
  ? cachedProtagonistRef
  : (globalHolder[MJ_PROTAGONIST_REF_KEY] = ref<Protagonist | null>(null)) as Ref<Protagonist | null>;
...
export class Protagonist extends Character {
   ...
   */
  static current: Ref<Protagonist | null> = protagonistRef;
  ...
  /**
   * 外部（如物品锻造界面）就地改写了装备 / 功法数据后，重算 HP/MP 上限并夹取当前值。
   *
   * `recomputeMaxHpMpAndClamp` 是 private 的内部钩子，外部无法直接调用；
   * 改词条会影响主属性与上限，必须提供此公开入口刷新派生值。
   */
  ...
  craftElixirFromMaterials(slotIndices: number[]): ElixirItemDefinition | null {
    ...
    // 阶层一致性：不同阶层的材料不可同炉（失败时不扣材料）
    const tierCheck = checkAlchemyTier(picks);
    if (!tierCheck.ok) return null;

    const elixir = craftElixirDef(picks.map((m) => ({ grade: m.grade, tier: m.tier })));
  ...
  applyTraitEffects(): void {
    for (const t of this.traits) {
      ...
      if (!effect) continue;
      const r = resolveTraitEffect(effect, this.realm.major);
  ...
  applyStateChanges(state: StateParsed): void {
    ...
    for (const item of state.itemAdds) {
      try {
        ...
        // 品阶：优先采用 AI 明确给出的品阶——花十灵石买的便宜货就该稳定是下品。
        // 旧逻辑对丹药完全丢弃 AI 的 grade 改为随机，导致同一笔小额交易
        // 一会儿下品、一会儿神品；此处校验后统一沿用，非法才回退随机。
        const aiGrade = parseGrade((item as unknown as Record<string, unknown>).grade);
        const grade = aiGrade ?? rollGrade(this.realm.major, this.realm.minor);
        const gradeConstraint: import("./types/gameConstants").GradeConstraint =
          aiGrade ? { pinned: aiGrade } : {};
        if (itemType === "功法") {
          ...
          const bonus = validBonus.has(bonusName)
            ...
            : {};
          const gfTier = rollItemTier(this.realm.major);
          if (gfTier === "凡人") {
            this.addToInventory(
              rollMortalMartialArt(grade, item.name ? item.name : undefined),
            );
            continue;
          }
          this.addToInventory({
        ...
        if (itemType === "丹药") {
          const parsed = parseStorageObject(item, this.realm.major, this.realm.minor, undefined, gradeConstraint);
        ...
        let newTier: import("./types/itemTier").ItemTier | undefined;
        if (itemType === "法宝") {
          newTier = rollItemTier(this.realm.major);
          fn = rollTreasureFunction(grade, newTier);
          se = rollTreasureSpecialEffect(grade);
        }
        // 材料：阶层决定能炼出什么阶层的丹药，故同样按当前境界定档
        else if (itemType === "材料") {
          newTier = rollItemTier(this.realm.major);
        }
        this.addToInventory({
  ...
  static fromJson(input: string | unknown): Protagonist | null {
    ...
    return new Protagonist({
      ...
      profile: normalizeProfile(o.profile, "manual"),
    });
  }
  ...
  static fromFateChoice(fc: FateChoiceResult): Protagonist {
    ...
    const p = new Protagonist({
      ...
      // 主角画像由玩家自行填写（AI 不写），故默认标记为 manual。
      profile: createEmptyProfile("manual"),
    });
