...
/**
 * 无主角时的占位文案，按开局阶段区分——
 * 一律显示「完成命运抉择后…」会掩盖「正在生成」与「生成失败」两种真实状态。
 */
const placeholderText = computed(() => {
  switch (storyStore.phase.value) {
    case "loading":
      return "正在生成开局剧情，请稍候…";
    case "error":
      return "开局剧情生成失败。请返回标题重新开一次人生，或检查 API 配置。";
    case "ended":
      return "此人生已终结，主角档案不再显示。";
    default:
      return "完成命运抉择后将在此显示主角档案。";
  }
});
...
const profileTarget = shallowRef<Protagonist | null>(null);

function openProfileModal() {
  profileTarget.value = props.protagonist ?? null;
  profileModalOpen.value = !!profileTarget.value;
}
  ...
  <section class="main-panel main-panel--player mj-pane--player" aria-label="主角信息">
    ...
    <div class="main-panel__body">
      <template v-if="!protagonist">
        <p class="main-panel__placeholder">{{ placeholderText }}</p>
      </template>
